...
Days_in_month_to_analyze <- days_in_month(Month_to_analyze)

month_vector <- str_pad(1:Days_in_month_to_analyze, pad = 0, width = 2, "left")
aqi_all <- data.frame()
for (i in month_vector) {
  API_query <- GET(paste0("http://www.airnowapi.org/aq/forecast/zipCode/?zipCode=19806&API_KEY=INSERT_API_KEY_HERE&date=2020-06-",
                            as.character(i),
...
as.character(Month_to_analyze_end), "T23&parameters=OZONE&BBOX=-75.83862,38.44498,-74.98992,39.84017&dataType=A&format=application/json&verbose=0&nowcastonly=0&includerawconcentrations=1&API_KEY=INSERT_API_KEY_HERE")

#API Call
Monitorin_data_DE <- GET(url = get_url)

# converting API results to DF
Monitorin_data_DE <-  fromJSON(rawToChar(Monitorin_data_DE$content))
Monitorin_data_DE

#selecting only the max daily AQI for the month
Monitoring_data_daily_max <- Monitorin_data_DE %>% 
  rename(c("Actual_AQI" = AQI)) %>% 
  mutate(Date = as.Date(Monitorin_data_DE$UTC)) %>% 
  group_by(Date) %>% 
  slice(which.max(Actual_AQI))
